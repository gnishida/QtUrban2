	...
	for (int i = 0; i < features.size(); ++i) {
		// avenue exからコピー
		{
			...
			for (boost::tie(vi, vend) = boost::vertices(features[i].roads(RoadEdge::TYPE_AVENUE).graph); vi != vend; ++vi) {
				...
				// 頂点を生成
				RoadVertexPtr v = RoadVertexPtr(new RoadVertex(pt));// + QVector2D(Util::genRand(-100, 100), Util::genRand(-100, 100))));
...
Polyline2D AliagaRoadGenerator::generatePolyline(const QVector2D& p1, const QVector2D& p2, float length) {
